...
**CodePoetryEditor**는 C/Java 스타일의 코드를 가져와  
텍스트 내용은 거의 건드리지 않은 채,  
**여백·줄바꿈·시각적 레이어만 과하게 조작하게 만드는 코드 포에트리 에디터**입니다.  
기능이 아니라 **리듬과 간격을 감상하는 도구**라는 컨셉으로 설계되었습니다.
...
- **실행 환경**: 최신 브라우저(Chrome, Edge 등)만 있으면 동작, 별도 설치/서버 불필요  
- **역할**:
  - 코드의 “의미”를 바꾸지 않고,  
    **공백·줄 간격·강조 레이어**를 통해 코드의 “형식”과 “리듬”을 새롭게 만드는 실험적 에디터
  - Mallarmé의 *주사위 던지기*처럼, 우연과 조판(레이아웃)을 코드에 적용해 보는 시도
